...
public static class OrleansExtensions
{
    public static void ConfigureSilo(HostBuilderContext context, ISiloBuilder siloBuilder)
    {
        ...
        {
            var dashboardPort =
                int.Parse(EnvironmentVariables.GetValue(context.Configuration, "ClusterDashboardPort",
                    "Orleans:DashboardPort"));
            siloBuilder.UseDashboard(options => { options.Port = dashboardPort; });
        }
    }
}