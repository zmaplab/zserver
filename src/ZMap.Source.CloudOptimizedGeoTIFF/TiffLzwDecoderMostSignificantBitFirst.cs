...
internal ref struct TiffLzwDecoderMostSignificantBitFirst
{
    ...
    {
        _allocatedBuffer =
            ArrayPool<byte>.Shared.Rent((TotalCodes - 256) * (sizeof(byte) + sizeof(byte) + sizeof(short)));
    ...
    public int Decode(ReadOnlySpan<byte> source, Stream destination)
    {
        int sourceOffset = 0;
        ...
        while (true)
        {
            ...
            else if (prefix == ClearCode)
            {
                ...
                destinationOffset++;
                // Console.WriteLine("DestinationOffset: " + destinationOffset);
            ...
            else
            {
                ...
                do
                {
                    var i = destinationOffset++;
                    // Console.WriteLine("I: " + i);
                    // destination[i] = rbp;
                    // Console.WriteLine("DestinationOffset: " + i);
                    destination.WriteByte(rbp);
                    rbp = ref Unsafe.Add(ref rbp, -1);
