...
ASSERT
RowCountVar <> 0, ''no rows affected, rollback'';
...
SELECT DeploymentIdArg,
       ...
       ProxyPortArg,
       StartTimeArg,
       IAmAliveTimeArg ON CONFLICT (DeploymentId, Address, Port, Generation) DO
            NOTHING;
...
SELECT ServiceIdArg,
       ...
       PeriodArg,
       GrainHashArg,
       0 ON CONFLICT (ServiceId, GrainId, ReminderName)
        DO
UPDATE SET
    ...
    RETURNING
    OrleansRemindersTable.Version
INTO STRICT VersionVar;
...
VALUES (''ReadReminderRowsKey'', ''
    SELECT
        ...
        StartTime,
        Period,
        Version FROM OrleansRemindersTable
    WHERE
...
ALTER
COLUMN Timestamp TYPE TIMESTAMPTZ(3) USING Timestamp AT TIME ZONE ''UTC'';

ALTER TABLE OrleansMembershipTable
ALTER
COLUMN StartTime TYPE TIMESTAMPTZ(3) USING StartTime AT TIME ZONE ''UTC'',
ALTER
COLUMN IAmAliveTime TYPE TIMESTAMPTZ(3) USING IAmAliveTime AT TIME ZONE ''UTC'';

-- Recreate routines

CREATE
OR REPLACE FUNCTION update_i_am_alive_time(
    deployment_id OrleansMembershipTable.DeploymentId%TYPE,
    address_arg OrleansMembershipTable.Address%TYPE,
    port_arg OrleansMembershipTable.Port%TYPE,
    generation_arg OrleansMembershipTable.Generation%TYPE,
...
CREATE
OR REPLACE FUNCTION insert_membership(
    DeploymentIdArg OrleansMembershipTable.DeploymentId%TYPE,
    AddressArg      OrleansMembershipTable.Address%TYPE,
    PortArg         OrleansMembershipTable.Port%TYPE,
    GenerationArg   OrleansMembershipTable.Generation%TYPE,
...
CREATE
OR REPLACE FUNCTION update_membership(
    DeploymentIdArg OrleansMembershipTable.DeploymentId%TYPE,
    AddressArg      OrleansMembershipTable.Address%TYPE,
    PortArg         OrleansMembershipTable.Port%TYPE,
    GenerationArg   OrleansMembershipTable.Generation%TYPE,
...
ALTER
COLUMN modifiedon TYPE TIMESTAMPTZ USING modifiedon AT TIME ZONE ''UTC'';
...
CREATE
OR REPLACE FUNCTION upsert_reminder_row(
    ServiceIdArg    OrleansRemindersTable.ServiceId%TYPE,
    GrainIdArg      OrleansRemindersTable.GrainId%TYPE,
    ReminderNameArg OrleansRemindersTable.ReminderName%TYPE,
    StartTimeArg    OrleansRemindersTable.StartTime%TYPE,
