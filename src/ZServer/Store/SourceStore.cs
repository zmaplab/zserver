...
namespace ZServer.Store
{
    ...
    public class SourceStore : ISourceStore
    {
        ...
        private ISource Get(string sourceName, JToken section)
        {
            ...
            if (string.IsNullOrWhiteSpace(provider))
            {
                _logger.LogError("数据源 {Name} 不存在或驱动为空", sourceName);
                return null;
            }
            ...
            if (type == null)
            {
                _logger.LogError("数据源 {Name} 的驱动 {Provider} 不存在", sourceName, provider);
                return null;
            }
            ...
            var parameterInfos = StorageTypeCache.GetOrAdd(type, x =>
            {
                if (!typeof(IVectorSource).IsAssignableFrom(x))
                {
                    _logger.LogError("数据源 {Name} 的驱动 {Provider} 不是有效的驱动", sourceName, provider);
                    return null;
                }
            ...
            else
            {
                ...
                {
                    _logger.LogError("创建数据源 {Name} 的驱动 {Provider} 失败: {Exception}", sourceName, provider,
                        e.ToString());
                    return null;
                }
            }
